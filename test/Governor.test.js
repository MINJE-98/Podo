...
contract("Governor", ([dev, alice, bob, carol, minje]) => {
    ...
    it('투표승리 후 출금 요청', async ()=>{
        ...
        // 제안 정보
        const propose = await governor.proposals(campaignId,proposeId);
        // 1. 실행이 상태값 확인
        // 2. 금고 포도 확인
        // 3. 포도 입금 확인
        // // 프로젝트 정보
        // const campaignInfo = await campaign.campaignList(campaignId);
        // //   현재 금고 잔액에서 출금 잔액을 뺌
        // assert(campaignInfo.currentMoney.toString() === propose.proposeAmount.toString());
        // // 그룹의 포도 수량 정보
